...
class DbPdo extends Db{
    ...
    public function __construct($config=''){
        if ( !class_exists('PDO') ) {
            Debug::addmsg('<font color=red>不支持的数据库驱动PDO，检查是否安装</font>');
        }
        if(!empty($config)) {
            $this->config   =   $config;
    ...
    /*
     *数据库配置信息转为dsn  mysql:dbname=frogphp;host=localhost
     * 
     */
    private function get_dsn($config){
    	//默认为mysql可以再这里进行修改
    	return "mysql:dbname={$config['database']};host={$config['hostname']}";
    }
    /**
     +----------------------------------------------------------
     * 释放查询结果
    ...
    public function getFields($tableName) {
        ...
        if($result) {
            foreach ($result as $key => $val) {
                $name= strtolower(isset($val['FIELD'])?$val['FIELD']:null);
                $info[$name] = array(
                    'name'    => $name ,
                    'type'    => $val['TYPE'],
