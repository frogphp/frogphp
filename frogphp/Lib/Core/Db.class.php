...
class Db{
	...
	public function factory() {
		...
		if(empty($db_config['dbms']))
			Debug::addmsg('不支持该数据库');
		// 数据库类型
		$this->dbType = ucwords(strtolower($db_config['dbms']));
		$class = 'Db'. $this->dbType;

		if(file_exists(FROG_DRIVER.$class.'.class.php')) {
			...
			// 类没有定义
			Debug::addmsg("没有这个类{$db_config['dbms']}");
		}
		return $db;
	}
	...
	protected function debug() {
		...
		if ( $this->debug ) {
			G('queryEndTime');
			Debug::addmsg('[用时<font color="red">'.G('queryStartTime','queryEndTime',6).'</font>秒] - ',2); //debug
			Debug::addmsg("<font color='green'>SQL 语句: ".$this->queryStr.'</font>',2);
		}
	}
			...
			foreach ($where as $key=>$val){
				...
				if(0===strpos($key,'_')) {
					...
					if(!preg_match('/^[A-Z_\|\&\-.a-z0-9]+$/',trim($key))){
						Debug::addmsg('字段未安全过滤 Db.class.php');
		...
		if(is_array($val)) {
			if(is_string($val[0])) {
				if(preg_match('/^(EQ|NEQ|GT|EGT|LT|ELT|NOTLIKE|LIKE)$/i',$val[0])) { // 比较运算
					...
					Debug::addmsg('Db.class.php parseWhereItem方法中');
